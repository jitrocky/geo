    ...
    if not content:
        ...
        with st.spinner("AI正在分析..."):
            try:
                ...
                规则：
                - 用输入内容的原语言重写成一篇完整、连贯的文案（长度类似原内容，约{len_chars}字符）。
...
# 页脚
st.sidebar.markdown("---")
st.sidebar.info("基于Python + Streamlit | 支持任意语言 | 升级：自然评分 + 维度分解（真实变化）")                - 整个报告用中文回复，简洁专业。
                内容：{content}
                """
                prompt_original = textwrap.dedent(prompt_original_raw)
